\begin{section}{Implementation and Power Spectra}
 ...
 the cross power spectrum $P_{\alpha\beta}(k)$ ($\alpha=\beta$ for auto power spectrum) is defined as
 \begin{align}
 ...
 $r_{\alpha\beta}(k) = P_{\alpha\beta}/\sqrt{P_{\alpha\alpha}P_{\beta\beta}}$.  In the right panel of
 ...
 and $\delta_L$ from \citet{bib:Yu2016}, where $\delta_E(\bs{q})= - \nabla_q \cdot \bs{\Psi}(\bs{q})$ is the 
 negative divergence of the real nonlinear displacement from simulations. 
 Ideally, the MM algorithm aims at getting the cross correlation $r_{RL}$ close to $r_{EL}$. 
