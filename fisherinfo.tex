\begin{section}{Fisher Information Content}
  ...
  we are currently computing the Fisher information given by $\delta_E$ \cite{bib:Yu2016} 
  ...
  to propagate final fields back to the initial conditions \citep{bib:HarnoisD2013}.
  This also eliminates the second increase of the information on small scales, which answers the question of section 4
  ({\it Information about what?}) of \citet{bib:HarnoisD2013}.

  Another way to quantify the nonlinear scale of $\delta_\alpha$
  is via the information plateau's linear equivalent scale, $k_{\rm p}$, satisfying
  \begin{equation}
      I_{\delta_L}(k_{\rm p})=I_{\delta_{\alpha}}(k\rightarrow\infty).
  \end{equation}
  In the left panel of Fig. \ref{fig:fisherinfo}, we can see that 
  $k_{\rm p}$ is just the scale on which 
  ...
  for $\delta_N$, $k_{\rm p}\simeq 0.15$ $h$/Mpc.
  The MM reconstruction increases $k_{\rm p}$ to $0.4$ $h$/Mpc,
